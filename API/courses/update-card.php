...
// initialize object
	...
	$card->user_id=$_REQUEST["user_id"];
	$card->card_id=$_REQUEST["card_id"];
	$card->chapter_id=$_REQUEST["chapter_id"];
    $card->card_ques=$_REQUEST["card_question"];
	$card->card_answer=$_REQUEST["card_answer"];
    ...
    try {
        ...
        //print_r($decoded);
    	    $card->uid = $decoded->data->id;
    	    
              if($card->user_id==$card->uid){
                $response=$card->updateCard();
                
                $uploaddocs->card_id=$card->card_id;
            ...
            /*  $stmt= $uploaddocs->getAllQuestionPdfWithCardID();
                $question_pdf_data =$stmt->fetchAll(PDO::FETCH_ASSOC);
    			$response["question_pdf"] =$question_pdf_data;   */
                }
             else{
